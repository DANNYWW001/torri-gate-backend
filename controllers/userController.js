...
const handleRegister = async (req, res) => {
  ...
  try {
    //check if user exists (checking for email and phoneNumber)
    //
    ...
    //send an email of thee verification token to the email
...
const handleLogin = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.status(401).json({ messaage: "Account Not found, Register" });
    }
    if (user.role !== role) {
      return res.status(403).json({ messaage: "Access Denied for this role" });
    }
    if (!user.isVerified) {
      return res
        .status(403)
        .json({ messaage: "Email Not verified, Check your mail" });
    }
    ...
    //jwt(json web tokens)
    //payload is used for unique idenitfication
